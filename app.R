...
stations <- stations[substr(stations, start = 7, stop = 8) == '_F']
...
melt_events <- tibble(read.csv("all_events.csv"))
...
#long table - panel 2
all_events_long <- read.csv('all_events_long.csv')
all_events_long$Date <- as.Date(all_events_long$Date)
all_events_long <-
  subset(
    all_events_long,
    all_events_long$method %notin%
...
events_month <- data.frame(month = sort(rep(1:12, 22)),
                           station = rep(unique(substr(
                             stations, start = 1, stop = 6
                           )), 12))
for (i in 1:length(models)) {
  for (j in 1:nrow(events_month)) {
    events_month[j, i + 2] <-
      sum(
        all_events$station == events_month$station[j] &
          all_events$month == events_month$month[j] &
          all_events$method == models[i]
      )
  }
}
colnames(events_month) <- c('month', 'station', models)
events_month <-
...
all_events_summary <- all_events %>%
  ...
  summarise(total_evetns = n())

all_events_summary$duration <-
  VLOOKUP(all_events_summary$station,
          data_stations,
          'Station',
          'Duration')
...
all_events_summary$latitude <- VLOOKUP(all_events_summary$station, data_stations, Station, Latitude)
...
conf_matrix <- function(df, ref) {
  ...
  for (i in 3:9) {
    # i = 3
    conf_mat <-
      confusionMatrix(data = factor(df[, i]), reference = factor(ref))
...
server <- function(input, output, session) {
    ...
    output$graph4 <- renderPlotly({
      plot_ly(complete_series_select, x = ~ Date) %>%
        add_lines(
          y = ~ spei,
          name = 'SPEI',
          line = list(color = user_palette[4])
        ) %>%
        add_lines(
          y = ~ sesr,
          name = 'SESR',
          line = list(color = user_palette[6]),
          visible = "legendonly"
        ) %>%
        add_lines(
          y = ~ eddi,
          name = 'EDDI',
          yaxis = 'y2',
          line = list(color = user_palette[7]),
          visible = "legendonly"
        ) %>%
        add_lines(
          y = ~ score_perc,
          name = 'Score',
          yaxis = 'y2',
          line = list(color = user_palette[9]),
          visible = "legendonly"
        ) %>%
        add_bars(
          y = ~ get(Method_1),
    ...
    output$graph6 <- renderLeaflet({
      leaflet() %>% addTiles() %>%
        ...
        addCircleMarkers(
          ...
          opacity = 0,
          fillOpacity = 1,
          popup = paste(data_stations$Station,
                        '<br>',
                        data_stations$IGPB2),
          fillColor = 'black'
        ) %>%
  ...
  observeEvent(input$update21, {
    ...
    # file2 = stations[1,1]
    # df <- read.csv(paste(file2, '_FLUXNET2015.csv', sep = ''))
    df <- read.csv(paste(file2, '.csv', sep = ''))
    # head(df)
    df <- df[, c(2, 4:9, 11)]
    colnames(df) <- c(
      "Date",
    ...
    df_long$order <- rep(1:7, nrow(df_long) / 7)
    
    # df_long$fd1 <- 'No'
    # df_long$fd1[df_long$fd == 0] <- 'No'
    # df_long$fd1[df_long$fd == 1] <- 'Yes'
    ...
    p_station <-
      ...
      # scale_fill_manual('Flash Drhought',values=c(No="#f0f0f0", Yes="#DE0000")) +
      ...
      ggtitle(paste('Flash Drought identification by all methods in ', file2, sep = '')) +
      xlab('Date') +
      ylab('Method') +
      theme(
  ...
  observeEvent(input$update31, {
    ...
    # selec1 <- stats[stats$Station == 'AT-Neu',]
    selec1 <- selec1[lubridate::month(selec1$Date) %in% 3:10, ]
    order <- sort(rep(1:6, 5))
    
    conf_mat1 <-
      conf_matrix(selec1, selec1$`Ford and Labosier`) %>%
  ...
  observeEvent(input$update32, {
    ...
    conf_mat2 <-
      conf_matrix(selec2, selec2$`Ford and Labosier`) %>%
      subset(.$model %notin% c('Mod. Ford and Labosier', 'Mo and Lettenmeier', 'Alencar et al.')) %>%
      cbind(order)
  ...
  observeEvent(input$update33, {
    ...
    conf_mat3 <-
      conf_matrix(selec3, selec3$`Ford and Labosier`) %>%
      subset(.$model %notin% c('Mod. Ford and Labosier', 'Mo and Lettenmeier', 'Alencar et al.')) %>%
      cbind(order)
  ...
  output$graph32 <- renderPlotly(
    plot_ly(
      ...
      # ) %>%
      layout(
        title = 'Number of events by each method at each lang use',
        yaxis = list(title = 'Number of events'),
        xaxis = list(title = 'Station'),
        # barmode = 'stack',
        plot_bgcolor = '#F0F0F0'
